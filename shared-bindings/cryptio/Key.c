...
static mp_obj_t cryptio_key_make_new(const mp_obj_type_t *type, size_t n_args,
    size_t n_kw, const mp_obj_t *all_args) {
    cryptio_aes_obj_t *self = mp_obj_malloc(cryptio_aes_obj_t, &cryptio_aes_type);
    ...
    common_hal_cryptio_aes_construct(self, key, key_length, iv, mode,
...
//|                                                         for CBC or CTR mode"""
//|         ...
static mp_obj_t cryptio_aes_rekey(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    cryptio_aes_obj_t *self = MP_OBJ_TO_PTR(pos_args[0]);
    enum { ARG_key, ARG_IV };
    static const mp_arg_t allowed_args[] = {
        {MP_QSTR_key, MP_ARG_OBJ | MP_ARG_REQUIRED, {.u_obj = MP_OBJ_NULL} },
        {MP_QSTR_IV, MP_ARG_OBJ, {.u_obj = MP_OBJ_NULL} },
    ...
    common_hal_cryptio_aes_rekey(self, key, key_length, iv);
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_KW(cryptio_aes_rekey_obj, 1, cryptio_aes_rekey);

static void validate_length(cryptio_aes_obj_t *self, size_t src_length,
    size_t dest_length) {
...
//|         ...
static mp_obj_t cryptio_aes_encrypt_into(mp_obj_t self_in, mp_obj_t src, mp_obj_t dest) {
    cryptio_aes_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ...
    common_hal_cryptio_aes_encrypt(self, (uint8_t *)destbufinfo.buf, destbufinfo.len);
    return mp_const_none;
}

static MP_DEFINE_CONST_FUN_OBJ_3(cryptio_aes_encrypt_into_obj, cryptio_aes_encrypt_into);
...
//|         ...
//|
static mp_obj_t cryptio_aes_decrypt_into(mp_obj_t self_in, mp_obj_t src, mp_obj_t dest) {
    cryptio_aes_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ...
    common_hal_cryptio_aes_decrypt(self, (uint8_t *)destbufinfo.buf, destbufinfo.len);
    return mp_const_none;
}

static MP_DEFINE_CONST_FUN_OBJ_3(cryptio_aes_decrypt_into_obj, cryptio_aes_decrypt_into);

static mp_obj_t cryptio_aes_get_mode(mp_obj_t self_in) {
    cryptio_aes_obj_t *self = MP_OBJ_TO_PTR(self_in);

    return MP_OBJ_NEW_SMALL_INT(self->mode);
}
MP_DEFINE_CONST_FUN_OBJ_1(cryptio_aes_get_mode_obj, cryptio_aes_get_mode);

static mp_obj_t cryptio_aes_set_mode(mp_obj_t self_in, mp_obj_t mode_obj) {
    cryptio_aes_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ...
    common_hal_cryptio_aes_set_mode(self, mode);
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_2(cryptio_aes_set_mode_obj, cryptio_aes_set_mode);

MP_PROPERTY_GETSET(cryptio_aes_mode_obj,
    (mp_obj_t)&cryptio_aes_get_mode_obj,
    (mp_obj_t)&cryptio_aes_set_mode_obj);

static const mp_rom_map_elem_t cryptio_locals_dict_table[] = {
    // Methods
    {MP_ROM_QSTR(MP_QSTR___name__), MP_OBJ_NEW_QSTR(MP_QSTR_AES)},
    {MP_ROM_QSTR(MP_QSTR_encrypt_into), (mp_obj_t)&cryptio_aes_encrypt_into_obj},
    {MP_ROM_QSTR(MP_QSTR_decrypt_into), (mp_obj_t)&cryptio_aes_decrypt_into_obj},
    {MP_ROM_QSTR(MP_QSTR_rekey), (mp_obj_t)&cryptio_aes_rekey_obj},
    {MP_ROM_QSTR(MP_QSTR_mode), (mp_obj_t)&cryptio_aes_mode_obj},
};
static MP_DEFINE_CONST_DICT(cryptio_locals_dict, cryptio_locals_dict_table);

MP_DEFINE_CONST_OBJ_TYPE(
    cryptio_aes_type,
    MP_QSTR_AES,
    MP_TYPE_FLAG_HAS_SPECIAL_ACCESSORS,
    make_new, cryptio_aes_make_new,
    locals_dict, &cryptio_locals_dict
    );
