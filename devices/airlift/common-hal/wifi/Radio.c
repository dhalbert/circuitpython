...
static void wait_for_ready(wifi_radio_obj_t *self, bool value, uint32_t timeout_ticks) {
...
static void spi_begin_transaction(wifi_radio_obj_t *self) {
...
// Internal routines that are also called from other classes.

// Send command via SPI. Assumes spi_begin_transaction() has already been called.
void wifi_radio_send_command(wifi_radio_obj_t *self, uint8_t cmd, const uint8_t **params, const size_t *param_lens, size_t num_params) {
...
size_t wifi_radio_wait_response_cmd(wifi_radio_obj_t *self, uint8_t cmd, uint8_t **responses, size_t *response_lens, size_t max_responses) {
    spi_begin_transaction(self);

    wifi_radio_wait_spi_char(self, _START_CMD);
    wifi_radio_check_data(self, cmd | _REPLY_FLAG);
    ...
    for (size_t i = 0; i < num_responses; i++) {
        uint8_t param_len = wifi_radio_read_byte(self);
    ...
    wifi_radio_check_data(self, _END_CMD);
...
size_t wifi_radio_send_command_get_response(wifi_radio_obj_t *self, uint8_t cmd,
    ...
    wifi_radio_send_command(self, cmd, params, param_lens, num_params);
    return wifi_radio_wait_response_cmd(self, cmd, responses, response_lens, max_responses);
}
...
void common_hal_wifi_radio_init_airlift(wifi_radio_obj_t *self, busio_spi_obj_t *spi, digitalio_digitalinout_obj_t *cs, digitalio_digitalinout_obj_t *ready, digitalio_digitalinout_obj_t *reset, digitalio_digitalinout_obj_t *gpio0) {
}
...
mp_obj_t common_hal_wifi_radio_get_version(wifi_radio_obj_t *self) {
    ...
    size_t num_resp = wifi_radio_send_command_get_response(self, _GET_FW_VERSION_CMD, NULL, NULL, 0, responses, response_lens, 1);

    if (num_resp > 0) {
        return mp_obj_new_str_from_cstr((char *)responses[0]);
    } else {
...
mp_obj_t common_hal_wifi_radio_get_hostname(wifi_radio_obj_t *self) {
    ...
    // }
    // return mp_obj_new_str_from_cstr(hostname);
    return mp_const_none;
}
...
void common_hal_wifi_radio_set_hostname(wifi_radio_obj_t *self, const char *hostname) {
    ...
    size_t param_lens[1] = { strlen(hostname) };
    uint8_t *responses[1];
    size_t response_lens[1];

    size_t num_resp = wifi_radio_send_command_get_response(self, _SET_HOSTNAME, params, param_lens, 1, responses, response_lens, 1);
...
mp_obj_t common_hal_wifi_radio_get_mac_address(wifi_radio_obj_t *self) {
    ...
    size_t num_resp = wifi_radio_send_command_get_response(self, _GET_MACADDR_CMD, params, param_lens, 1, responses, response_lens, 1);

    if (num_resp > 0 && response_lens[0] >= MAC_ADDRESS_LENGTH) {
        // Reverse byte order (ESP32 returns it reversed)
        for (int i = 0; i < MAC_ADDRESS_LENGTH; i++) {
            mac[i] = responses[0][MAC_ADDRESS_LENGTH - 1 - i];
        }
        m_free(responses[0]);
    } else {
