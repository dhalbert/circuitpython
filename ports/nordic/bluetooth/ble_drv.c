...
#define CASE_RETURN(evt) case evt: \
        return MP_STRINGIFY(evt)

static const char *ble_drv_evt_name(uint32_t evt) {
    switch (evt) {
        CASE_RETURN(BLE_GAP_EVT_CONNECTED);
        CASE_RETURN(BLE_GAP_EVT_DISCONNECTED);
        CASE_RETURN(BLE_GAP_EVT_CONN_PARAM_UPDATE);
        CASE_RETURN(BLE_GAP_EVT_SEC_PARAMS_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_SEC_INFO_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_PASSKEY_DISPLAY);
        CASE_RETURN(BLE_GAP_EVT_KEY_PRESSED);
        CASE_RETURN(BLE_GAP_EVT_AUTH_KEY_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_LESC_DHKEY_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_AUTH_STATUS);
        CASE_RETURN(BLE_GAP_EVT_CONN_SEC_UPDATE);
        CASE_RETURN(BLE_GAP_EVT_TIMEOUT);
        CASE_RETURN(BLE_GAP_EVT_RSSI_CHANGED);
        CASE_RETURN(BLE_GAP_EVT_ADV_REPORT);
        CASE_RETURN(BLE_GAP_EVT_SEC_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_CONN_PARAM_UPDATE_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_SCAN_REQ_REPORT);
        CASE_RETURN(BLE_GAP_EVT_PHY_UPDATE_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_PHY_UPDATE);
        CASE_RETURN(BLE_GAP_EVT_DATA_LENGTH_UPDATE_REQUEST);
        CASE_RETURN(BLE_GAP_EVT_DATA_LENGTH_UPDATE);
        CASE_RETURN(BLE_GAP_EVT_QOS_CHANNEL_SURVEY_REPORT);
        CASE_RETURN(BLE_GAP_EVT_ADV_SET_TERMINATED);

        CASE_RETURN(BLE_GATTC_EVT_PRIM_SRVC_DISC_RSP);
        CASE_RETURN(BLE_GATTC_EVT_REL_DISC_RSP);
        CASE_RETURN(BLE_GATTC_EVT_CHAR_DISC_RSP);
        CASE_RETURN(BLE_GATTC_EVT_DESC_DISC_RSP);
        CASE_RETURN(BLE_GATTC_EVT_ATTR_INFO_DISC_RSP);
        CASE_RETURN(BLE_GATTC_EVT_CHAR_VAL_BY_UUID_READ_RSP);
        CASE_RETURN(BLE_GATTC_EVT_READ_RSP);
        CASE_RETURN(BLE_GATTC_EVT_CHAR_VALS_READ_RSP);
        CASE_RETURN(BLE_GATTC_EVT_WRITE_RSP);
        CASE_RETURN(BLE_GATTC_EVT_HVX);
        CASE_RETURN(BLE_GATTC_EVT_EXCHANGE_MTU_RSP);
        CASE_RETURN(BLE_GATTC_EVT_TIMEOUT);
        CASE_RETURN(BLE_GATTC_EVT_WRITE_CMD_TX_COMPLETE);

        CASE_RETURN(BLE_GATTS_EVT_WRITE);
        CASE_RETURN(BLE_GATTS_EVT_RW_AUTHORIZE_REQUEST);
        CASE_RETURN(BLE_GATTS_EVT_SYS_ATTR_MISSING);
        CASE_RETURN(BLE_GATTS_EVT_HVC);
        CASE_RETURN(BLE_GATTS_EVT_SC_CONFIRM);
        CASE_RETURN(BLE_GATTS_EVT_EXCHANGE_MTU_REQUEST);
        CASE_RETURN(BLE_GATTS_EVT_TIMEOUT);
        CASE_RETURN(BLE_GATTS_EVT_HVN_TX_COMPLETE);
